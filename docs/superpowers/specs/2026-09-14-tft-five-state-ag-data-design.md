...
按照 MMST-ViT 官方 `sentinel_loader.py` 的选择方式，从每月 1 日和 15 日的影像中保留每月第一幅，形成六个时间点：
...
其中 `G` 是县内 9 km 网格数量。本阶段不要求 AG 网格顺序与气象网格建立融合映射，但必须保存 FIPS、年份、日期、网格数量，并验证六个月的 AG 网格数量一致。
...
- `ag_images`：`(6, G, 3, 224, 224)`；
- `ag_dates`：六个 `(month, day)` 时间点；
- `FIPS`；
- `Year`；
- `State`；
- `County`；
