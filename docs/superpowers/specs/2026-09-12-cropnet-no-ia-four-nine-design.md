...
数据提取和所有 manifest 生成使用显式允许州集合，只保留以下八州：
...
验收只检查最终样本记录、缓存条目和 manifest 中的州值是否全部属于这八州，不做源代码子串审查。通过允许列表自然排除其他州。
...
- 项目数据提取和 manifest 生成使用八州允许列表；
- 最终 JSONL、网格缓存元数据和 official JSON 中的州值全部属于八州允许列表；
- 不存在旧的 275 步默认配置；
