...
本章按照“联合消融、独立测试、基线比较、提前预报”的顺序组织实验。$2\times2$ 联合消融表明，逐网格 VSN 的平均主效应在 2021 和 2022 年均降低 RMSE；空间注意力的作用则依赖变量选择位置，在逐网格 VSN 条件下进一步改善预测。逐网格 VSN 与时空位置增强的缩放点积空间注意力组合在两个年份均取得最优结果，季末 RMSE 分别为 18.990 bu/ac（2021 年验证集）和 20.086 bu/ac（2022 年独立测试集）。提前预报结果显示，至 8 月 31 日模型的测试 $R^2$ 已达到 0.535，表明模型能够在季中提取与最终产量相关的时序信息。
...
本文仍存在以下局限。第一，模型仅使用气象和县级土壤信息，尚未纳入遥感植被指数、品种、播期、灌溉和田间管理等变量，因此对管理措施差异和作物长势的直接刻画仍然不足。第二，县级土壤被表示为县均值静态特征，无法描述县域内部土壤剖面的空间变化。第三，实验目前采用单个验证年和单个独立测试年，虽然 2022 年结果支持模型的跨年泛化能力，但仍需要更多年份滚动验证或跨区域测试来评估稳定性。第四，基线模型采用各自原生的特征变换和预设训练配置，当前比较尚未进行完整的逐样本统计显著性检验，因此性能差异的统计可靠性仍需进一步确认。第五，GNN-RNN 的测试图依据测试县的地理坐标独立构建，属于不使用测试标签的传导式空间评估；在严格的归纳式部署场景中，还需要研究训练区到新县域的图结构迁移方法。
